...
**Architecture in one line:** a fully static Astro build (`npm run build` → `dist/`) served by **Cloudflare Pages**, connected to a **GitHub** repo, with content edited through the **admin editor** at `/admin/new` (news/events) and **Sveltia CMS** at `/admin`. Every push to the repo's main branch triggers an automatic rebuild.
...
### Admin editor — `/admin/new`

Alongside the Sveltia CMS at `/admin`, the site ships a **single-page admin editor** at **`/admin/new`** (source: `src/pages/admin/new.astro`) — a two-role login (admin + editor, email + password) plus a one-page news/events editor, backed by **Cloudflare Pages Functions** in the repo's `functions/` directory:

- `/api/content` — unified CRUD for news + events (GET list/one, POST/PUT/DELETE). Every write commits markdown to GitHub via the Contents API — the same commit-triggers-rebuild loop as the CMS — so a saved article appears on the site within a minute.
- `/api/upload` — image uploads to `public/images/` (5MB cap, image extensions only).
- `/api/media` — the media library the editor's picker reads.
- `/api/preview` — gated draft preview (session cookie required).

The editor flow: a **create dialog** forces a name + author/organiser and creates a **random-slug draft** (e.g. `/news/x7k2m9p4`); **Save is explicit** (no autosave); **Preview** opens the last saved version; **Duplicate** copies to a fresh draft; images are auto-converted to **WebP** client-side (min 400px wide, 800px+ recommended), thumbnails are **cropped to 16:9**, body images have a size selector and text blocks a **Wide** toggle. **Publishing is gated**: an article is public only when `ready: true` and `now >= date` (date = publish time). Only admins can tick **Ready** or **Publish now**; editors can create/edit/save drafts. The editor page itself is public (it holds no secrets); the API is the security boundary, gated by an HMAC-signed HttpOnly session cookie.
...
| `ADMIN_PASSWORD` | Admin login password (keep it strong) |
| `EDITOR_EMAIL` | Editor login email (draft-only account) |
...
- [ ] `/admin/new` signs in with both admin and editor credentials.
